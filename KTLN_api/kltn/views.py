...
from kltn.perms import IsPermitUser, IsPermitUserGivePoint
...
class DiemDetailViewset(viewsets.ViewSet, generics.RetrieveAPIView):
    ...
    def post_diem(self, request, pk):
        ...
        if not khoaluan.hoidong.thanhviens.filter(id=request.user.id).exists() and request.user.is_superuser != 1:
            return Response({'ERROR:':'Nguoi danh gia phai thuoc hoi dong'})
        try:
